...
const contextMenuStringTable: IContextMenuStringTable = {
...
export class ContextMenuBuilder {
  /**
   * Matches a string with international characters
   *
   * @param word
   * @private
   */
  private static matchesWord(word: string) {
   ...
   * add an ellipsis.
   *
   * @param  word {string}   The string to truncate
   * @return result {string} The truncated string
   */
  private static truncateString(word: string) {
    const match = word.match(/^.{0,25}[\S]*/);
    const length = (match && match[0].length) || 0;
    let result = match && match[0].replace(/\s$/, '');
   ...
   */
  constructor(webContents, processMenu) {
    this.processMenu = processMenu;
    this.stringTable = { ...contextMenuStringTable };

    webContents = webContents || BrowserWindow.getFocusedWindow();
    ...
    this.getWebContents = () =>
      webContents ? webContents : BrowserWindow.getFocusedWindow()!.webContents;
  }
   ...
   *
   */
  public setAlternateStringFormatter(stringTable) {
    this.stringTable = { ...this.stringTable, ...stringTable };
  }
   ...
   */
  public async showPopupMenu(contextInfo) {
   ...
   */
  public async buildMenuForElement(info) {
   ...
   */
  public async buildMenuForTextInput(menuInfo) {
    const menu = new Menu();
   ...
   * @return {Menu}  The `Menu`
   */
  public buildMenuForText(menuInfo) {
    const menu = new Menu();
   ...
   * @return {Menu}  The `Menu`
   */
  public buildMenuForImage(menuInfo) {
    const menu = new Menu();
   ...
   */
  public addSearchItems(menu, menuInfo) {
    if (!menuInfo.selectionText || menuInfo.selectionText.length < 1) {
      return menu;
    }

    const match = ContextMenuBuilder.matchesWord(menuInfo.selectionText);
    ...
    if (process.platform === 'darwin') {
      ...
      const lookUpDefinition = new MenuItem({
        label: this.stringTable.lookUpDefinition({
          word: ContextMenuBuilder.truncateString(menuInfo.selectionText),
        }),
  ...
  /**
   * Verifies if the src URL is exists
   *
   * @param menuInfo
   */
  public isSrcUrlValid(menuInfo) {
