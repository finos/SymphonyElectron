...
const REPLACE_COMMAS = new RegExp(',', 'g');
...
class Search {
    ...
    /**
     * This is the magic part of this library
    ...
    static constructQuery(query, senderId, threadId) {
        ...
        if (/"/.test(query)) {
            q += (query.match(/"(.*?)"/g)).toString().replace(REPLACE_COMMAS, ' ');
            q += " ";
        }

        let hashCashTagQuery = "(";
        tokens.forEach((item, i) => {
            if (i > 0 && (item.startsWith('#') || item.startsWith('$')) && hasToken) {
                hashCashTagQuery += " OR ";
            }
