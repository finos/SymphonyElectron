...
class WebDriver {
    ...
    async  login() {
        await this.inputText(ui.SIGN_IN_EMAIL, specconst.USER_A.username);
        await this.inputText(ui.SIGN_IN_PASSWORD, specconst.USER_A.password);
        var singin = await this.getElementByXPath(ui.SIGN_IN_BUTTON);
        await singin.click();
        await this.waitElelmentIsNotVisible(ui.SIGN_IN_BUTTON);
    }
    ...
    async  startDriver() {
        ...
        .manage()
        .window().getSize();    
        await this.driver.get(specconst.ROOURL);     
    }
